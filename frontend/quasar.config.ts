...
export default defineConfig((/* ctx */) => {
  return {
    ...
    extras: [
      ...
      // No Roboto: the app loads Atkinson Hyperlegible Next in app.scss instead.
    ...
    build: {
      ...
      vueRouterMode: 'history', // clean URLs; Laravel's catch-all route serves index.html for every path outside /api and /s/{token}
      // vueRouterBase,
      ...
      // distDir stays at its default (dist/spa) on purpose. Pointing it straight at
      // ../backend/public looks tidier but is destructive: before every build Quasar runs
      // removeBuildArtifacts(), an fse.removeSync() on the whole distDir — it would delete
      // index.php and every Filament asset under css/filament, js/filament and
      // fonts/filament. It is not a Vite setting, so build.emptyOutDir cannot switch it off.
      // "pnpm build" therefore builds here and then copies into Laravel's public/ with
      // scripts/copy-to-laravel.mjs, which only ever removes files Quasar itself owns.
    ...
    devServer: {
      ...
      // vueDevtools: true,
      // https: true,
      open: false, // the harness's own Browser pane opens it, not a spawned OS browser
